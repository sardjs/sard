...
import { Point, SyntheticTouches, SyntheticTouch } from './types'
...
export interface Fingers {
  ...
  update(touches: SyntheticTouches, callback?: (...args: any[]) => any): void
  track(knock: Knock, callback?: (...args: any[]) => any): void
  remove(touches: SyntheticTouches): void
...
export function createFingers(
  ...
  function add(
    ...
    touches.forEach((touch: SyntheticTouch) => {
      if (firstFinger === -1) {
  ...
  // 跟踪一段时间内手指在屏幕上移动的信息
  function track(knock: Knock, callback?: (...args: any[]) => any) {
  ...
  // 移除掉不在屏幕上的手指的信息
  // touchend 事件触发时调用
  function remove(touches: SyntheticTouches) {
