...
export interface Chopsticks {
  ...
  degrees: number
  isPaired(): boolean
  update(touches: SyntheticTouches): boolean
  remove(
  ...
  getDistance(): number
  getAngle(): number
  isMove(touches: SyntheticTouches): boolean
...
export function createChopsticks(knocks: Knocks): Chopsticks {
  ...
  function update(/*changedTouches*/ touches: SyntheticTouches) {
  ...
  // 判断移动的是否是筷子
  function isMove(touches: SyntheticTouches) {
